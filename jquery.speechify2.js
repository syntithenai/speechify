...
;(function($) {
	var methods={
		init : function(options) {
			...
			function traverseGrammar(parts,current,word,i,grammar) {
			...
			function addGrammarsInner(rule,grammar,activeGrammar) {
				...
				for (var i=0 ; i< parts.length; i++) {
					...
					if (word.slice(0,1) == "[" && word.slice(-1) == "]" ) {
						...
						// traverse the rest of this grammar without this token
						for (var j=i+1 ; j< parts.length; j++) {
							current2 = traverseGrammar(parts,current2,parts[j],j,grammar);
						}
					...
					current = traverseGrammar(parts,current,word,i,grammar);
				};
			}
			...
			function addGrammars(grammars,activeGrammars)  {
				...
				//console.log(['grammar',grammar]);
					$.each(grammar.texts,function(ruleKey,rule) {
						addGrammarsInner(rule,grammar,activeGrammars);
					});
				});
