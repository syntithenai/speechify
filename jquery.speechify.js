...
;(function($) {
			...
			try {
				...
				function startRecognising() {
					...
					if (!recognising) {
						...
						$('#speechify-status').html('<span class="microphone microphone-on"><img alt="on" src="'+$.fn.speechify.relPath+'images/microphone.png" ></span>').show();
						$('.voice-help').show();
						recognising=true;
						if (!handlerStarted) {
	...
	$.fn.speechify.say=function(text) {
		if (typeof speak == 'function') {
			speak(text,{ pitch: 50 ,speed:150,wordgap:2});
		} else {
			console.log('need speak.js libs');
		}	
	};
